...
- `src/minion.rs` - Minion lifecycle and management
- `src/workspace.rs` - Git worktree operations
- `src/stream.rs` - Claude Code JSON stream parser
...
- `/respond <pr# or URL>` - Respond to all comments/reviews on a PR
...
- Use `just test-verbose` for verbose test output
...
- See `experiments/DMX_ANALYSIS.md` for approach comparison
