...
2. Select highest priority — four tiers (`priority:critical` > `priority:high` > `priority:medium` > `priority:low`; unlabeled falls between medium and low); ties broken by oldest-first (FIFO)
