...
- `build_interactive_command()` — required to implement (no default body, deliberately mirroring `build_interactive_resume_command` so a new backend has to make an explicit choice); construct a fresh interactive session with a custom system prompt for `gru chat`/`gru pm`/`gru tpm`, or return `None` to opt out (callers then surface an actionable error naming the command and agent). Its `github_host` argument, when `Some`, must be set as `GH_HOST` on the command so `gh` inside the session targets the right GitHub instance (matching `build_interactive_resume_command`); when `None`, leave the variable alone so `gh` falls back to its own configuration rather than being pinned to a guess. If you implement it, also update the suggested-agent list in `interactive_unsupported_error()` (`src/agent_registry.rs`) — like the `do` command's `--agent` help, it enumerates backends by hand rather than deriving them
